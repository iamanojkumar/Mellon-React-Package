...
**Where the roadmap stands**: SPEC phases 1–17 and 19–29 are shipped (~148 exported components) — Phase 19 shipped `Video`/`Audio` only, and moved its remaining items into Phase 18's backlog slot. Only **Phase 18 remains as backlog** — Mobile Gestures (Pull To Refresh, Swipe Actions) plus the rest of the old Phase 19 (Scroll Area, Split Pane, Resizable, Infinite Scroll, Masonry). Everything else marked "next" in this file is either blocked on the Foundation palette (see the Chart track) or a deliberate non-goal.
