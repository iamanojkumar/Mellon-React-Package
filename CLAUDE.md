...
**Where the roadmap stands**: SPEC phases 1–17 and 20–29 are shipped (~146 exported components). Only **phases 18–19 remain as backlog** — Mobile Gestures (Pull To Refresh, Swipe Actions) and Remaining Utilities & Media (Scroll Area, Split Pane, Resizable, Infinite Scroll, Masonry, Video, Audio). Neither depends on the other. Everything else marked "next" in this file is either blocked on the Foundation palette (see the Chart track) or a deliberate non-goal.
...
- **`src/styles/` never enters the library build.** `styles/index.css` (which pulls in `variables.css` + `reset.css`) is imported only by `.storybook/preview.tsx`, so `dist/style.css` _consumes_ `var(--ds-*)` everywhere but _defines_ nothing, and `files: ["dist"]` doesn't ship the sources either. Storybook and the tests look fine because Storybook imports it directly. Consumers currently have to supply the token layer themselves from `@mellon-design/tokens-web`. Whether that's the intended contract or a packaging bug is undecided — don't "fix" it by importing CSS into `src/index.ts` without deciding, since that makes the tokens unavoidable for consumers who already load the Foundation.
...
Standing gaps and audit findings live in `docs/TOKEN_AUDIT.md` and `docs/CHART_TOKEN_REQUIREMENTS.md`; both carry reproduction commands. Verify a token still exists before citing either.
