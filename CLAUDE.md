...
**Where the roadmap stands**: SPEC phases 1–17 and 19–38 are shipped (~155 exported components) — Phase 19 shipped `Video`/`Audio` only, and moved its remaining items into Phase 18's backlog slot. Phases 30–38 (`SegmentTrack`; `Canvas`'s `renderBackdrop`/controlled `viewport` plus `Panel`; a consumer gap sweep; `Canvas` floating chat; `Canvas` snap/focus/fill; `Document`/`DocumentPage`; the `Node` graph family; a second consumer gap sweep; `CanvasChatPanel` history/resize/context) are single-addition, real-consumer-request work, not part of the dependency-ordered roadmap — see `docs/SPEC.md`'s per-phase notes and `docs/COMPONENT_LIST.md`. Only **Phase 18 remains as backlog** — Mobile Gestures (Pull To Refresh, Swipe Actions) plus the rest of the old Phase 19 (Scroll Area, Split Pane, Resizable, Infinite Scroll, Masonry). Everything else marked "next" in this file is either blocked on the Foundation palette (see the Chart track) or a deliberate non-goal.
