...
**Where the roadmap stands**: SPEC phases 1–17 and 19–30 are shipped (~149 exported components) — Phase 19 shipped `Video`/`Audio` only, and moved its remaining items into Phase 18's backlog slot. Phase 30 (`SegmentTrack`) is a single-component addition prompted by a real consumer request, not part of the dependency-ordered roadmap — see `docs/SPEC.md`'s Phase 30 note and `docs/COMPONENT_LIST.md`'s Media section. Only **Phase 18 remains as backlog** — Mobile Gestures (Pull To Refresh, Swipe Actions) plus the rest of the old Phase 19 (Scroll Area, Split Pane, Resizable, Infinite Scroll, Masonry). Everything else marked "next" in this file is either blocked on the Foundation palette (see the Chart track) or a deliberate non-goal.
...
**No icon library** — every icon is a small inline SVG; shapes used by multiple components are exported once and reused (e.g. `AlertVariantIcon` shared by `Alert`/`Banner`/`Toast`), never as a standalone icon with no owning component. This is a deliberate, standing rule, not an oversight — real consumer icon needs this scope doesn't cover (record/mic, folder, settings, save, …) are tracked in `docs/icons.md` rather than built silently. Read that file before adding a shared icon export; building one is its own deliberate scope decision (see the `src/icons/` note above), not something to fold into an unrelated task.
