...
| Component          | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                     | Key props                                                                                                                                                                                                                                                                                   |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Canvas`           | The workspace: viewport, selection, gestures, outline.                                                                                                                                                                                                                                                                                                                                                                                           | `scene`/`defaultScene`/`onSceneChange`, `onCommand`, `selectedIds`, `grid`, `outlineVisible`, `shapeToolbar`, `readOnly`, `renderBlock`, `viewport`/`defaultViewport`/`onViewportChange`, `renderBackdrop`, `aiPrompt` `aiPromptFloating` `chatContext` `aiRewrite` `aiCluster` `aiDiagram` |
| `CanvasBlock`      | Positioned, selectable wrapper + face; resizable except for the node-like kinds (see below). Independently exported/usable — not only rendered by `Canvas`'s own loop.                                                                                                                                                                                                                                                                           | `block`, `selected` `highlighted` `editing` `resizable` (ignored for `node`/`sticky`/`shape`), `onInputPortClick` `onOutputPortClick` `nodeConnecting` (connection ports), `chrome` (a `document` block's `Document.chrome` override — see below), `renderBlock`                            |
...
| `StickyNote`       | Tinted note, editable in place.                                                                                                                                                                                                                                                                                                                                                                                                                  | `text` `tone` `color` (arbitrary hex fill, layers over `tone`'s accent edge — user content, not a token), `editing` `onTextChange` `onEditingEnd`, `aiRewrite` `buildAIPrompt` `aiRewriteLabel` `aiRewriteEditable`                                                                         |
| `CanvasShape`      | Flowchart vocabulary via `clip-path`. No separate `circle` kind — `shape="ellipse"` with equal width/height renders as one. Editable in place the same way `StickyNote` is — double-click swaps its label for an input.                                                                                                                                                                                                                          | `shape` (`rectangle` `ellipse` `diamond` `triangle` `parallelogram`), `text` `tone` `color` (same free-fill escape hatch as `StickyNote`), `editing` `onTextChange` `onEditingEnd`                                                                                                          |
...
| `CanvasFillPicker` | Trigger + `Popover` for a block's `color` — preset swatches plus a full `ColorPicker`. Shown by `CanvasBlock` only while a `sticky`/`shape`/`node` block is selected.                                                                                                                                                                                                                                                                            | `value` `onChange`, `presets` (defaults to `DEFAULT_CANVAS_FILL_PRESETS`), `triggerLabel`                                                                                                                                                                                                   |
| `CanvasToolbar`    | Small floating bar for inserting a sticky note, shape, node or frame by hand — no `AIProvider`/resolver needed, unlike every other Canvas affordance. Rendered by `Canvas` via `shapeToolbar`.                                                                                                                                                                                                                                                   | `onInsert` (`CanvasInsertKind`)                                                                                                                                                                                                                                                             |

### Block kinds

`sticky`, `text`, `image`, `shape`, `node`, `divider`, `embed`, `frame`, plus
`code` (`code`, `language`), `table` (`columns`, `rows`, `caption`), `link`
(`url`, `title`, `description`), `checklist` (`title`, `items`), `chart`
(`label`, `data`, `chartType`), and `document` (`pages`, `aspectRatio`,
`layout`, `header`, `footer` — see `Document`/`DocumentPage` below).

All but `checklist`, `document`, and `node` are delegation to components that
already exist — `Code`, `Table`, `Link`, `ChartSurface`, `Image`, `Divider` —
so they add no folders. `checklist` is the exception because it is the one
face with state of its own; a tick becomes an `update` command through the
reducer, the same path a model's change takes. `document` delegates to
`Document` itself (`chrome={false}` by default — overridable per `CanvasBlock`
instance via its own `chrome` prop, for a host that wants one specific
document-kind block to drop a user into `Document`'s standalone, self-
contained viewer, e.g. a "focused page" view independent of `Canvas`'s own
pan/zoom; `Canvas` itself never sets it, so nothing changes unless a consumer
opts in on a `CanvasBlock` it renders directly), and is the one kind with a
dedicated double-click behaviour: opening its editor enters `Canvas`'s own
focus mode, locked by default (see "Navigating the canvas" below). A short
table row renders as empty cells rather than a ragged grid, and a `link`
always carries `rel="noopener noreferrer"`.

`node` (`name`, `color`, `hasInput`, `hasOutput`) delegates to the standalone
`Node` component (`fill` mode — see the Node section below), reusing
`Canvas`'s own connector system for the actual arrows rather than
`NodeConnector`, which only exists inside a standalone `NodeGraph`. Its
output/input ports are click-driven exactly like `NodeGraph`'s own: click a
source's output port to arm it, then a target's input port to complete a
`connect` command through the same reducer every other mutation goes
through; Escape cancels an armed connection before falling through to
`Canvas`'s usual Escape handling (focus mode, then selection).

### Node-like kinds — `node`, `sticky`, `shape`
...
- **They are connectable.** All three draw `Node`'s input/output port dots and
  ...
  `shape` draw their ports at the `CanvasBlock` level (a port sits half
  outside the face's box, and `diamond`/`triangle` clip anything inside them),
  and only when `onInputPortClick`/`onOutputPortClick` are supplied — so a
  `readOnly` canvas shows no dots at all. `node` keeps drawing inert ports
  either way, since there the dots also state the graph's shape.
- **Selection is a rounded highlight**, not the rectangular resize frame the
  sized kinds carry — a frame with corner points that resize nothing is a lie
  about what the block does. The wrapper carries `data-node-like` for styling.

`Canvas`'s `aiRewrite` only wires up a "Rewrite with AI" trigger for `sticky`
and `shape` — the two kinds with a click-to-edit text entry point on the
canvas. Every other kind has no such entry point today, so the prop is
silently a no-op for them rather than a half-working affordance. A `shape`'s
trigger renders at the `CanvasBlock` level (a selection-gated overlay, same
corner as `CanvasFillPicker`'s trigger on the opposite side), not inside
`CanvasShape` itself — a trigger drawn inside a clipped shape
(`diamond`/`triangle`/`parallelogram`) would be clipped away with it.
...
summary instead, attached to that reply's own turn in the history — a
"Thinking" heading, then one CSS-truncated line of the text itself; there's
no control that reveals more of it than that one line, and no animated
indicator tied to it (an earlier version's dots kept spinning after the
reply had already landed — the only busy indicator now is a separate
`TypingIndicator` shown while a request is actually in flight, gone the
moment it settles). Only `submit` populates `thinking` —
...
| Component             | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Key props                                                                                                                                                                                                    |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `CanvasPromptBar`     | Prompt input; `@` references a block by id, or a host-supplied `references` entry (a page, a document — anything that isn't a canvas block), listed under its own `referenceLabel` heading rather than as a block. `variant="minimal"` drops the border/background, the horizontal padding, and the Send button (Enter still submits) — for a chrome-light host like `CanvasChatPanel` rather than a toolbar row; `"default"` (unchanged) elsewhere. `minimal` deliberately keeps the **vertical** padding `size` asks for: it is about removing chrome so the text sits flush with the host's own edge, not about collapsing the field to one line of text height, which used to leave the variant meant for a chat composer as the one that couldn't be taller than a toolbar input.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `blocks` `references` `referenceLabel` `onSubmit`, `status` `error` `disabled` `variant` `size`                                                                                                              |
| `CanvasChangePreview` | Staged diff with per-command accept/reject.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `scene` `commands` `rejected` `message` `onAccept` `onReject`                                                                                                                                                |
| `CanvasChatPanel`     | Draggable, resizable, minimizable floating chat over the canvas — `Canvas`'s `aiPromptFloating` decouples `CanvasPromptBar` from the static row into this instead. Never closable — minimize by double-clicking the header, its hover/focus-revealed icon button, or an opt-in `minimizeShortcut` chord. Keeps the full exchange, not just the latest turn: every `lastMessage` that changes to a new value is appended to a scrollable history, oldest first, alongside the prompt that led to it. Resize by dragging the corner handle, or Alt+Arrow (Shift for a bigger step) while any focusable part of the panel has focus. `context` folds anything the consumer wants the model to see — app state, the signed-in user, a page's own metadata — into every prompt alongside the selection; a plain string rides verbatim, anything else is JSON-serialized. `selectedBlocks` (and so both the prompt's embedded context and the chip row) already includes a selected frame's members, expanded by `Canvas` via `withFrameMembers` before the prop reaches this component — the panel itself just renders whatever list it's given. The selection is named chip-by-chip up to `MAX_SELECTION_CHIPS`, then collapses to one "N items selected" chip. | `blocks` `references` `referenceLabel` `selectedBlocks` `onSubmit`, `promptSize`, `status` `error` `lastMessage` `lastMessageVariant` `thinking` `context` `title` `minimizeShortcut` `disabled` `boundsRef` |
...
**`pages` strings are HTML, not markdown.** `<p>Bold: <strong>yes</strong></p>`, never `**yes**` — markdown written into a page renders as literal asterisks. Worth stating in whatever prompt drives a model writing into `pages`, the same way the canvas command vocabulary has to be. Same trust model as `CanvasBlock`'s `text` kind: content the consumer already owns, not third-party markup (that belongs in a sandboxed `embed`).
...
clipped box and moves whatever doesn't fit onto the following page, appending
one when there isn't one. It runs whether the content came from a keystroke or
from anywhere else — a `pages` prop, a host's own update, an AI writing a long
body in one shot — and it runs whether or not `editable` is set, since content
that doesn't fit is content the reader can't see either way. One page moves per
pass; the effect re-runs on the resulting `pages` and picks the content up
wherever it landed.

Two deliberate limits. It only ever flows _forward_ — deleting a paragraph
never pulls the next page's content back onto the page that now has room. And
it splits _between_ top-level blocks, never inside one, so a single block
taller than a whole page stays where it is and clips; moving it would push the
same block from page to page forever without it ever fitting
(`paginationSplitIndex` in `src/utilities/documentPagination.ts`, which is the
whole policy, pure and unit-tested — jsdom has no layout engine, so keeping the
arithmetic separate from the measuring is what makes any of it testable).

**A consequence worth stating outright, because it looks identical to the
pagination being broken: a page whose HTML is a single wrapper element never
paginates.** `<div><h2>…</h2><ul>…</ul>…</div>` has exactly one top-level
block, that block is the page's first, and a page's first block is the one
thing that never moves — so the whole document clips at page one while the
table of contents happily lists headings the reader can't reach. Write the
blocks as siblings (`<h2>…</h2><ul>…</ul>…`) and it flows. This bites hardest
when a model generates the page body, since nothing about "use real tags"
tells it not to wrap the result; a consumer generating `pages` should either
...
`setTimeout` rather than `requestAnimationFrame` (rAF never fires at all in a
backgrounded/unpainted tab, a real failure mode for a canvas app switched away
from mid-paste, not just a test-environment quirk). A split caused by a `pages`
update moves nobody's focus.

`DocumentPage.Body` forwards its ref specifically so this measurement is
possible — and `Document`'s own `.editor` wrapper around `RichTextEditor` uses
`min-height: 100%`, not `height: 100%`: a fixed height would silently absorb
the overflow right there, before it ever reaches the body node this reads.
...
A graph of connectable, groupable nodes — a standalone family, not a `Canvas`
block kind, so it (and its data) can be used and referenced from any module.
A node's `data` is `unknown`: it can hold a string, a form value, or an entire
scene parsed from elsewhere (a `Canvas` `scene`, a `Document`'s `pages`) —
`NodeGraph` never inspects it, only positions the box and hands it to
`renderNode`.

Connecting node A's output to node B's input doesn't merge or copy anything at
connect time. `computeNodeOutput(nodeId, nodes, connections)`
(`utilities/nodeGraph.ts`) derives B's effective output — `{ [A.id]: A.data,
[B.id]: B.data }` — on every read, keyed by node id rather than assuming
`data` is a spreadable object, so it works for any shape and always reflects
the current graph. That function, `NodeGraphData`, and the rest of
`nodeGraph.ts`'s pure helpers are the public surface other modules read a
graph through — there's no internal state only `NodeGraph` can drive.

**Connecting is click-driven, not drag-driven**: click a node's output port to
arm it, then click a target's input port to complete the connection (or
...
`canConnect`/`wouldCreateCycle` reject a self-connection, a duplicate, or
anything that would close a loop, checked before the connection is made so
`computeNodeOutput`'s recursion never has to guard against one in practice.

**Repositioning stays pointer-drag-only** (drag a node by its header), with
arrow keys as the keyboard equivalent once a node is selected — the same
split `Canvas` draws between spatial dragging and keyboard navigation.
Shift-click adds to the selection; with 2+ nodes selected, `G` groups them
into a new `NodeGroupData` (`onUngroup` dissolves a group without touching its
members); Delete removes selected nodes along with any connection touching
them and drops them from any group.

**Grouping is data, not geometry** — a `NodeGroupData.nodeIds` list, unlike
`CanvasFrame`'s geometric containment — so `NodeGroup`'s drawn box
(`groupBounds`) is a rendering of that membership, not the source of truth
for it.

`Node` renders as a fully-rounded pill/chip by default (an arbitrary hex
`color` fill drops its border/shadow for a flat look, the same free-fill
escape hatch as `StickyNote`/`CanvasShape`'s `color`), and only grows a
divided body below its name when it actually holds `children` — a bare
label chip stays a single row. `fill` swaps its self-positioning
(`style.left`/`style.top`, the default `NodeGraph` uses) for filling
whatever already-positioned wrapper it sits in, which is what lets it also
be `Canvas`'s `node` block kind (see below) without a second implementation.

| Component       | What it does                                                                                      | Key props                                                                                          |
| --------------- | ------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `NodeGraph`     | The graph: renders nodes, groups and connections; owns selection, dragging, connecting, grouping. | `value`/`defaultValue`/`onChange` (`NodeGraphData`), `renderNode`, `readOnly`                      |
| `Node`          | One positioned, nameable pill with an input port (left) and output port (right).                  | `id` `name`, `onRename`, `selected` `hasInput` `hasOutput` `connecting` `color` `fill`, `children` |
| `NodeConnector` | One SVG edge between two already-resolved port points — pure geometry, nothing measures the DOM.  | `from` `to` (points), `selected`, `onSelect`                                                       |
| `NodeGroup`     | The visible dashed boundary a group draws around its members, with a renamable label.             | `name`, `onRename`, `onUngroup`                                                                    |
...
| `AISuggestionPopover` | The standard result surface: streaming text, error/retry, accept/reject. `editablePrompt` + `onSubmit` swap the default "fetch on open" for an editable textarea pre-filled from `editablePrompt`, so the end user (not just the integrating developer) can steer the instruction before it's sent — used by `StickyNote`'s `aiRewriteEditable`. | `triggerLabel` **required**, `status` `result` `error`, `onAccept`/`onReject`/`onRetry` (omit `onAccept` for read-only explanations), `editablePrompt` `onSubmit` `promptLabel` `submitLabel`, `placement` |
...
| `ThinkingBlock`        | Collapsible reasoning trace.                      | `thinking` — "Thinking…" while set, "Thought for 8s" once it clears; `duration` (seconds) overrides the block's own measurement of that transition; `label` overrides both (default "Show reasoning"); `open`/`onOpenChange` |
