...
| `FileUpload`     | Drop zone + file list. Presentation only: it owns no upload transport.                  | `files` `onFilesAdded` `onRemove` `onReject`, `accept` `multiple` `maxSize` (bytes), `aiDescribe`                                                             |
